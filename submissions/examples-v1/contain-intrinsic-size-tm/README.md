...
A self-contained EaseMotion-css submission that demonstrates the `contain intrinsic size` pattern using the framework's design tokens.

## Features

- Default `.contain-intrinsic-size` rule backed by `--ease-color-primary-alpha` and `--ease-color-primary-dark`
...
```html
<div
  class="contain-intrinsic-size contain-intrinsic-size-md contain-intrinsic-size-primary"
>
  Hello world
</div>
```
...
- `style.css` — All rules for `.contain-intrinsic-size` and variants, plus layout, dark mode, and reduced motion.
