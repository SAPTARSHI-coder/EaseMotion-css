...
3.  **The `row-reverse` Trick:** In HTML, the 5th star comes _first_, followed by the 4th, 3rd, 2nd, and 1st. We place them in a flex container with `flex-direction: row-reverse`. This visually displays them from 1 to 5, left to right.
4.  **Why `row-reverse`?:** CSS sibling selectors (`~`) only select elements that come _after_ the current element in the DOM. By reversing the DOM order, when you hover or click the 3rd star, the 2nd and 1st stars actually come _after_ it in the HTML structure. This allows us to use `label:hover ~ label` and `input:checked ~ label` to "fill in" all the stars to the left of the interacted one.
