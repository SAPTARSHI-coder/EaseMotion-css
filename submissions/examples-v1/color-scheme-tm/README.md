...
A self-contained EaseMotion-css submission that demonstrates the `color scheme` pattern using the framework's design tokens.

## Features

- Default `.color-scheme` rule backed by `--ease-color-primary-alpha` and `--ease-color-primary-dark`
...
- `style.css` — All rules for `.color-scheme` and variants, plus layout, dark mode, and reduced motion.
