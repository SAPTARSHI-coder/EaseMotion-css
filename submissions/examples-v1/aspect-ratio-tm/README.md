...
A self-contained EaseMotion-css submission that demonstrates the `aspect ratio` pattern using the framework's design tokens.

## Features

- Default `.aspect-ratio` rule backed by `--ease-color-primary-alpha` and `--ease-color-primary-dark`
...
- `style.css` — All rules for `.aspect-ratio` and variants, plus layout, dark mode, and reduced motion.
