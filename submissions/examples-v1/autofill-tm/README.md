...
A self-contained EaseMotion-css submission that demonstrates the `autofill` pattern using the framework's design tokens.

## Features

- Default `.autofill` rule backed by `--ease-color-primary-alpha` and `--ease-color-primary-dark`
...
- `style.css` — All rules for `.autofill` and variants, plus layout, dark mode, and reduced motion.
