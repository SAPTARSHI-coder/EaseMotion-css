...
A self-contained EaseMotion-css submission that demonstrates the `animation timeline` pattern using the framework's design tokens.

## Features

- Default `.animation-timeline` rule backed by `--ease-color-primary-alpha` and `--ease-color-primary-dark`
...
```html
<div
  class="animation-timeline animation-timeline-md animation-timeline-primary"
>
  Hello world
</div>
```
...
- `style.css` — All rules for `.animation-timeline` and variants, plus layout, dark mode, and reduced motion.
