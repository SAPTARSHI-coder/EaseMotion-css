...
A self-contained EaseMotion-css submission that demonstrates the `animation composition` pattern using the framework's design tokens.

## Features

- Default `.animation-composition` rule backed by `--ease-color-primary-alpha` and `--ease-color-primary-dark`
...
```html
<div
  class="animation-composition animation-composition-md animation-composition-primary"
>
  Hello world
</div>
```
...
- `style.css` — All rules for `.animation-composition` and variants, plus layout, dark mode, and reduced motion.
