...
A self-contained EaseMotion-css submission that demonstrates the `container queries` pattern using the framework's design tokens.

## Features

- Default `.container-queries` rule backed by `--ease-color-primary-alpha` and `--ease-color-primary-dark`
...
```html
<div class="container-queries container-queries-md container-queries-primary">
  Hello world
</div>
```
...
- `style.css` — All rules for `.container-queries` and variants, plus layout, dark mode, and reduced motion.
