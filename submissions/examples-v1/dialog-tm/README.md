...
A self-contained EaseMotion-css submission that demonstrates the `dialog` pattern using the framework's design tokens.

## Features

- Default `.dialog` rule backed by `--ease-color-primary-alpha` and `--ease-color-primary-dark`
...
- `style.css` — All rules for `.dialog` and variants, plus layout, dark mode, and reduced motion.
