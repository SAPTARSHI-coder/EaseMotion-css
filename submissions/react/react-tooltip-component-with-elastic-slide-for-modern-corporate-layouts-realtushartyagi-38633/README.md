...
## Files
- `Tooltip.jsx` – The React component. It handles the internal state (`isVisible`) via mouse and focus events, ensuring accessibility.
- `style.css` – Contains the custom CSS properties, the corporate aesthetics (clean borders, dark slate bubble), and the `cubic-bezier` timing function logic that creates the elastic slide effect.
- `demo.html` – A standalone HTML file demonstrating how the tooltip looks and feels in a standard browser environment.

## Installation
1. Copy the `Tooltip.jsx` and `style.css` files into your React project (e.g., `src/components/Tooltip/`).
...
|----------|---------------|-------------|
| `--ease-tooltip-timing` | `0.4s` | The duration of the elastic slide entrance animation. |
| `--ease-tooltip-easing-bounce` | `cubic-bezier(0.34, 1.56, 0.64, 1)` | The precise bezier curve that dictates the "spring" physics of the slide. |
| `--ease-corp-tooltip-bg` | `#1e293b` | The dark slate background color of the tooltip bubble. |
