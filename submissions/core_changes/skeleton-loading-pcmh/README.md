...
Skeleton loading placeholders with shimmer animation. Shape variants: text, circle (avatar), rectangle (image), card, and table rows.
...
| Image | `.ease-skeleton` + custom height | Rectangle (card image, article) |
...
- `demo.html` — Profile card, article, card grid, and table skeleton demos
