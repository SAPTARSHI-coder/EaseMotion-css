...
A self-contained EaseMotion-css submission that demonstrates the `is selector` pattern using the framework's design tokens.

## Features

- Default `.is-selector` rule backed by `--ease-color-primary-alpha` and `--ease-color-primary-dark`
...
- `style.css` — All rules for `.is-selector` and variants, plus layout, dark mode, and reduced motion.
