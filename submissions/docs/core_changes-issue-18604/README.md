...
A self-contained EaseMotion-css submission that demonstrates the `placeholder shown` pattern using the framework's design tokens.

## Features

- Default `.placeholder-shown` rule backed by `--ease-color-primary-alpha` and `--ease-color-primary-dark`
...
```html
<div class="placeholder-shown placeholder-shown-md placeholder-shown-primary">
  Hello world
</div>
```
...
- `style.css` — All rules for `.placeholder-shown` and variants, plus layout, dark mode, and reduced motion.
