...
A self-contained EaseMotion-css submission that demonstrates the `where selector` pattern using the framework's design tokens.

## Features

- Default `.where-selector` rule backed by `--ease-color-primary-alpha` and `--ease-color-primary-dark`
...
```html
<div class="where-selector where-selector-md where-selector-primary">
  Hello world
</div>
```
...
- `style.css` — All rules for `.where-selector` and variants, plus layout, dark mode, and reduced motion.
