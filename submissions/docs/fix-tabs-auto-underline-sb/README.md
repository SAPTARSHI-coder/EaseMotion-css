...
### 3. Why is it useful?
The transform rules on lines 106-125 have higher specificity and include `display: block` through the shorthand. Without `!important`, the `display: none` rule gets overridden, causing the underline to still appear when using the `ease-tabs-auto` variant.
