...
A self-contained EaseMotion-css submission that demonstrates the `mask image` pattern using the framework's design tokens.

## Features

- Default `.mask-image` rule backed by `--ease-color-primary-alpha` and `--ease-color-primary-dark`
...
```html
<div class="mask-image mask-image-md mask-image-primary">
  Hello world
</div>
```
...
- `style.css` — All rules for `.mask-image` and variants, plus layout, dark mode, and reduced motion.
