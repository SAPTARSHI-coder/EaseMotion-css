...
Using `*` to force `animation-duration: 0.01ms` is an accessibility anti-pattern because it kills _all_ transitions on a webpage, including harmless color/opacity transitions. This submission ensures that `EaseMotion CSS` respects the user's OS settings by stripping nausea-inducing motion (`transform`, `translate`, `scale`) while elegantly falling back to opacity fades (`ease-kf-fade-in` / `ease-kf-fade-out`) where applicable. This provides a vastly superior user experience for those with motion sensitivity.
