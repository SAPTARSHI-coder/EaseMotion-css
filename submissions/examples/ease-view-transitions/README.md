...
## What does this do?
A two-page demo showcasing the CSS View Transitions API for cross-document (MPA) navigation. A gallery card "morphs" into the detail page hero using shared `view-transition-name` elements.

## How is it used?
Open `gallery.html` in a Chromium-based browser (Chrome/Edge 126+) and click any card. The browser captures the old state, transitions to the new page, and animates the shared element.

## How it works
- `@view-transition { navigation: auto }` enables cross-document transitions
- Cards in `gallery.html` use `view-transition-name: card-N` on their image
...
| File | Purpose |
|------|---------|
| `gallery.html` | Gallery page with 3 cards, each linking to detail |
