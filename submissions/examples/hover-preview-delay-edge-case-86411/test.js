...
export class HoverPreviewCard {
  constructor(trigger, preview, options = {}) {
    ...
    this.showDelay =
      typeof options.showDelay === "number" && options.showDelay >= 0
        ? options.showDelay
        : 300;
    this.hideDelay =
      typeof options.hideDelay === "number" && options.hideDelay >= 0
        ? options.hideDelay
        : 200;
    this.showTimer = null;
    this.hideTimer = null;
...
describe("Hover Preview Card Delay Timeout Edge Cases", () => {
  ...
  it("should fallback to default delay threshold when negative delay is supplied", () => {
    const instance = new HoverPreviewCard(trigger, preview, {
      showDelay: -500,
    });
