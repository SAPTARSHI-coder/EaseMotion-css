...
A highly accessible, WCAG 2.1 AA compliant Interactive Card component designed to behave like a native button while allowing complex HTML nesting.

## Accessibility Features

- **Keyboard Interaction**: Custom `keydown` event listener strictly mimics native button behavior. Users can activate the card using both the `Enter` and `Space` keys. `event.preventDefault()` is used on the Space key to prevent the browser from scrolling down the page.
