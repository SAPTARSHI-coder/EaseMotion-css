...
_Note: As browsers do not allow writing to the system clipboard via pure CSS, developers consuming this library should attach a standard JavaScript `navigator.clipboard.writeText()` listener to `.ease-copy-btn` to enable actual clipboard functionality._
...
- **Syntax-Theme Agnostic:** It styles the _container_, not the _tokens_. You can drop any syntax highlighter into the `<code>` tag without color clashes.
