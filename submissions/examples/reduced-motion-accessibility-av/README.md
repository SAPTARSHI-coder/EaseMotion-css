...
_Note: We strictly use `0.01ms` instead of `0s` because setting the duration to pure zero can catastrophically break JavaScript event listeners that are waiting for `transitionend` or `animationend` events to fire before updating the DOM._
