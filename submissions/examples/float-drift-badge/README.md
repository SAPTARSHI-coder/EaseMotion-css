# Float-Drift Badge for SaaS Showcase Layouts
...
```text
submissions/examples/float-drift-badge/
├── demo.html    # Semantic HTML5 layout demonstrating SaaS showcase and badges
├── style.css    # CSS custom properties, keyframe animations, and responsive styles
...
The floating drift motion is achieved using the `@keyframes float-drift` rule:
...
- **Staggered Delays**: Different float instances use `animation-delay` offsets (e.g., `-1.2s`, `-2.8s`) to create an organic, non-synchronous drifting appearance across multiple badges.
- **Hover Behavior**: When a user hovers over a badge, `animation-play-state: paused` holds the float position while smoothly applying a scale boost (`scale(1.03)`) and glowing border shadow.
