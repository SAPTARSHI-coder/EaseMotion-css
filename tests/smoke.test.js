...
describe("EaseMotion-css Smoke Tests", () => {
  ...
  it("should not activate modals with URL hash selectors", () => {
    expect(css).not.toContain(".ease-modal-overlay:target");
  });

  it("should open and close modals without changing the URL hash", () => {
    const modalScript = readFileSync(
      resolve(__dirname, "../core/modal.js"),
      "utf8"
    );
    const modalDom = new JSDOM(
      `<!DOCTYPE html>
      <html>
        <body>
          <button id="open-modal" data-modal-target="#demo-modal">Open Modal</button>
          <div id="demo-modal" class="ease-modal-overlay">
            <div class="ease-modal">
              <button id="close-modal" data-modal-close>Close</button>
            </div>
          </div>
        </body>
      </html>`,
      {
        ...
        url: "https://example.com/page#demo-modal",
      }
    );

    modalDom.window.eval(modalScript);
    modalDom.window.document.dispatchEvent(
      new modalDom.window.Event("DOMContentLoaded", { bubbles: true })
    );

    const overlay = modalDom.window.document.getElementById("demo-modal");
    const opener = modalDom.window.document.getElementById("open-modal");
    const closer = modalDom.window.document.getElementById("close-modal");

    expect(overlay.classList.contains("is-active")).toBe(false);
    expect(modalDom.window.location.hash).toBe("#demo-modal");

    opener.dispatchEvent(
      new modalDom.window.MouseEvent("click", {
        bubbles: true,
        cancelable: true,
      })
    );
    ...
    expect(modalDom.window.document.body.style.overflow).toBe("hidden");
    expect(modalDom.window.location.hash).toBe("#demo-modal");

    closer.dispatchEvent(
      new modalDom.window.MouseEvent("click", {
        bubbles: true,
        cancelable: true,
      })
    ...
    expect(overlay.classList.contains("is-active")).toBe(false);
    expect(modalDom.window.document.body.style.overflow).toBe("");
    expect(modalDom.window.location.hash).toBe("#demo-modal");
  });
