...
describe("EaseMotion-css Smoke Tests", () => {
  ...
  beforeAll(() => {
    ...
    const scrollProgress = readFileSync(
      resolve(componentsDir, "scroll-progress.css"),
      "utf8"
    );
    ...
    const tooltips = readFileSync(
      resolve(componentsDir, "tooltips.css"),
      "utf8"
    );
    const modals = readFileSync(resolve(componentsDir, "modals.css"), "utf8");
    const commandPalette = readFileSync(
      resolve(componentsDir, "command-palette.css"),
      "utf8"
    );
    const viewTransitions = readFileSync(
      resolve(componentsDir, "view-transitions.css"),
      "utf8"
    );
    ...
    const skeleton = readFileSync(
      resolve(componentsDir, "skeleton.css"),
      "utf8"
    );
    const scrollGallery = readFileSync(
      resolve(componentsDir, "scroll-gallery.css"),
      "utf8"
    );
    const readMore = readFileSync(
      resolve(componentsDir, "read-more.css"),
      "utf8"
    );
    const progress = readFileSync(
      resolve(componentsDir, "progress.css"),
      "utf8"
    );
    const passwordStrength = readFileSync(
      resolve(componentsDir, "password-strength.css"),
      "utf8"
    );
    const pagination = readFileSync(
      resolve(componentsDir, "pagination.css"),
      "utf8"
    );
    ...
    const connectionStatus = readFileSync(
      resolve(componentsDir, "connection-status.css"),
      "utf8"
    );
    const compareTable = readFileSync(
      resolve(componentsDir, "compare-table.css"),
      "utf8"
    );
    const btnMagnetic = readFileSync(
      resolve(componentsDir, "btn-magnetic.css"),
      "utf8"
    );
    const breadcrumb = readFileSync(
      resolve(componentsDir, "breadcrumb.css"),
      "utf8"
    );
    const avatar = readFileSync(resolve(componentsDir, "avatar.css"), "utf8");
    const announceBar = readFileSync(
      resolve(componentsDir, "announce-bar.css"),
      "utf8"
    );
    const pulseRing = readFileSync(
      resolve(__dirname, "../animations/pulse-ring.css"),
      "utf8"
    );
  ...
  it("minified bundle should be valid and contain key classes", () => {
    const bundle = readFileSync(
      resolve(__dirname, "../easemotion.min.css"),
      "utf8"
    );
  ...
  it("should have pulse ring classes and keyframes defined", () => {
    expect(css).toContain(".em-pulse-ring");
    expect(css).toContain(".em-fast");
    expect(css).toContain(".em-slow");
    expect(css).toContain("@keyframes em-pulse-ring-kf");
  });
});
